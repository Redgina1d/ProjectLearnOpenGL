...
public class InitialTest implements ILogic {
	...
	InitialTest() {
		...
		kekw = new Light(new Vector3f(10.0f,5.5f,0.0f), new Vector3f(0.5f,1.9f,1.2f));
	...
	public void init() throws Exception {
		...
		Model sunModel = loader.loadOBJModel("uv_sphere", "YELLOW");
		Model cubeModel = loader.loadOBJModel("uv_sphere", "touchgrass");
		Model surfaceModel = loader.loadOBJModel("detailed_surface", "green");
		Model skyboxModel = loader.loadOBJModel("skybox", "sky");
		Model pointModel = loader.loadOBJModel("cubepoint", "red");
		Model point2Model = loader.loadOBJModel("cubepoint", "white");
		Model bricks = loader.loadOBJModel("Cube", "brick_wall");
		...
		crosshair = new Entity(guiModel, new Vector3f(0, 0, 0), 0.0f, 0.1f);
	...
	public void input() {
		camInc.set(0, 0, 0);
		if(window.isKeyInteract(GLFW.GLFW_KEY_W))
			camInc.z = -1;
		if(window.isKeyInteract(GLFW.GLFW_KEY_S))
			camInc.z = 1;
		
		if(window.isKeyInteract(GLFW.GLFW_KEY_A))
			camInc.x = -1;
		if(window.isKeyInteract(GLFW.GLFW_KEY_D))
			camInc.x = 1;
		
		if(window.isKeyInteract(GLFW.GLFW_KEY_SPACE))
			camInc.y = 1;
		if(window.isKeyInteract(GLFW.GLFW_KEY_LEFT_SHIFT))
			camInc.y = -1;
		
		if(window.isKeyInteract(GLFW.GLFW_KEY_SLASH))
			if (keyCD == 0) {
				if(consoleOpened) {
					consoleOpened = false;
					keyCD = 100;
	...
	public void update(float interval, MouseInput mouseInput) {
		...
		if (i < 0.99f) {
			i += 0.01f;
			GLFW.glfwSetWindowOpacity(window.getWindowHandle(), i);
		} else {
			GLFW.glfwSetWindowOpacity(window.getWindowHandle(), 1);
